...
//		"WHEN (SELECT c FROM tab2 WHERE d = 2) = 3 THEN 'AAA' " +
	...
	public static void outPlayE(Expression expression , Map<String, String> map) {
		if ((expression instanceof AndExpression ) || (expression instanceof OrExpression ) ||  (expression instanceof Parenthesis )) {
//			System.out.println("type:"+expression.getClass());
			if((expression instanceof Parenthesis )){
				outPlayE(    (((Parenthesis) expression).getExpression())    ,map);
				return;
			}
			
			outPlayE(((BinaryExpression) expression).getLeftExpression() ,map);
			outPlayE(((BinaryExpression) expression).getRightExpression() ,map);
			
		} else if( (expression instanceof EqualsTo  )  && "?".equals(((BinaryExpression) expression).getRightExpression().toString())) {
			map.put(((BinaryExpression) expression).getLeftExpression().toString(), ((BinaryExpression) expression).getRightExpression().toString());
...
////		System.out.println(allComparisonExpression.GetSubSelect() );
//		
